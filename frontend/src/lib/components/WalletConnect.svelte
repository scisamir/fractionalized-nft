...
>
  ...
  <div
    class={`mr-menu-list absolute z-50 w-60  rounded-b-md border text-center shadow-sm backdrop-blur ${hideMenuList && "hidden"} ${isDark ? `bg-primary-950  text-neutral-50` : `bg-neutral-50 text-neutral-950`}`}
  >
...
{#snippet menuItem(icon: string | undefined, onclick: () => void, name: string)}
  <button
    class="flex h-16 cursor-pointer items-center px-4 w-full py-2 opacity-80 hover:opacity-100"
    {onclick}
  >
    {#if icon}
      <img
